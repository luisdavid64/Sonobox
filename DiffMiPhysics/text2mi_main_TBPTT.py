...
def text2mi(
    ...
    bptt_s = 0.25
    bptt_len = int(bptt_s * fs)
    T = int(seconds*fs)
    no_iters = seconds/bptt_s
    print(f"BPTT length: {bptt_len}")
    ...
    for n in pbar:
        ...
        while start < T:
            ...
            # print(f"Signal MI shape: {signal_mi.shape}")
            # print(f"Length of BPTT: {bptt_len}")
            total_audio[start:start+bptt_len] = signal_mi.squeeze().detach().cpu()
            start += bptt_len
            mass_spring_model.detach_state(reset_to_rest=False)
        ...
        if n % log_audio_every_n == 0:
            # Save audio
            sf.write(save_dir / f'optim_{n}.wav', total_audio.numpy(), 16000)
