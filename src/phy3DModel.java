...
public class phy3DModel extends PhyModel {
 ...
 public void init() {
  ...
  public void setGeometry(double d) { // vr: constraint for interaction elements: the distance between nodes and the initial length is the same
    m_dist = d;
    m_l0 = d; 
  }
 ...
 public ArrayList<Driver3D> addDrivers (List<String> InNodes) {
  ...
  for (String drvNode : InNodes) {
        ...
        drivers.add(driver);
        System.out.println(this.getName() + ": creating driver with name: " + driverName + " at node: " + drvNode + " stored as:: " + driver);
        count+=1;
    }
  ...
  public ArrayList<Observer3D> addListeners(List<String> OutNodes) {
    ...
    for (String obsNode : OutNodes) {
        ...
        System.out.println(this.getName() + ": creating listener with name: " + obsName + " at node: " + obsNode+ " stored as:: " + listener);
        count += 1;
    }
 ...
 public void generate () {
   //System.out.println(this.getName() + ": creating mass elements with naming pattern: " + m_mLabel + "_[X]_[Y]_[Z]");
   //System.out.println(this.getName() + ": creating interaction elements with naming pattern: " + m_iLabel + "_[X1]_[Y1]_[Z1]_[X2]_[Y2]_[Z2]");
...
private void generateDilated2WithFrame() {
  ...
  private void generateCliques() {
        ...
        for (int cz = 0; cz < cliqueZ; cz++) {
          ...
          for (int a = 0; a < nodes.size(); a++) {
            ...
            for (int b = a + 1; b < nodes.size(); b++) {
              ...
              int dy = n2[1] - n1[1];
              int dz = n2[2] - n1[2];
              addInteractions(n2[0], n2[1], n2[2], n1[0], n1[1], n1[2], masName1, dx, dy, dz, Math.max(Math.abs(dx), Math.max(Math.abs(dy), Math.abs(dz))));
            }
          }
        }
      }
    ...
    // 2. Connect each clique to its neighbors by a single edge (from the boundary-most node in this clique to the boundary-most node in the neighbor clique)
    for (int cx = 0; cx < cliqueX; cx++) {
      for (int cy = 0; cy < cliqueY; cy++) {
      for (int cz = 0; cz < cliqueZ; cz++) {
        // For each direction, connect the boundary-most node of this clique to the boundary-most node of the neighbor clique
        // X neighbor clique
        if (cx + 1 < cliqueX) {
